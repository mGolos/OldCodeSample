...
if not adressExists('./DG/x_%i.npy'%p1):
    ...
    dir_ = '../SearchForPatt/DG_pC/Cand_S'
    pars = findParameters(dir_)
    ksix = zeros((0,998))
    ksiA = zeros((0,998))
    for f0 in pars['f0']:
        pattAd = dir_ + '/patterns_P_0.60_f0_%.2f.npy' %f0
    ...
    ############################################################################################'
    # **Save**
    array2data(array(x), './DG/x_%i.npy'%p1); del x
    array2data(array(A), './DG/A_%i.npy'%p1); del A
